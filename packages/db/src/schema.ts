...
export const assignmentKindEnum = pgEnum("assignment_kind", [
  "PRIMARY",
  "COLLABORATOR",
]);
export const reviewShareStatusEnum = pgEnum("review_share_status", [
  "DRAFT",
  "SHARED",
  "ACTIVE",
  "REVOKED",
...
export const organizationSettings = pgTable(
  "organization_settings",
  {
    ...
    reviewShareExpiryDays: integer("review_share_expiry_days")
      .notNull()
      .default(30),
    intakeRetentionDays: integer("intake_retention_days")
      .notNull()
      .default(365),
    ...
    timesheetEditWindowDays: integer("timesheet_edit_window_days")
      .notNull()
      .default(7),
    immediateEmailEvents: text("immediate_email_events")
      .array()
      .notNull()
  ...
  (table) => [
    check(
      "retention_positive",
      sql`${table.intakeRetentionDays} > 0 AND ${table.draftRetentionDays} > 0 AND ${table.auditRetentionDays} > 0`,
    ),
  ],
);
...
export const integrationConnections = pgTable(
  ...
  (table) => [
    uniqueIndex("integration_org_kind_provider").on(
...
export const profiles = pgTable(
  ...
  (table) => [
    uniqueIndex("profile_email_lower_unique").on(sql`lower(${table.email})`),
  ],
);
...
export const memberships = pgTable(
  "memberships",
  {
    ...
    sessionRevokedAfter: timestamp("session_revoked_after", {
      withTimezone: true,
    }),
  ...
  (table) => [
    uniqueIndex("membership_org_profile_unique").on(
      table.organizationId,
      table.profileId,
    ),
    index("membership_org_role_idx").on(
      table.organizationId,
      table.role,
      table.status,
    ),
    check(
      ...
      sql`${table.accountType} <> 'TEMPORARY' OR ${table.expiresAt} IS NOT NULL`,
    ),
  ],
);
...
export const capacitySchedules = pgTable(
  "capacity_schedules",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
...
export const capacityExceptions = pgTable(
  "capacity_exceptions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
  ...
  (table) => [
    uniqueIndex("capacity_exception_member_date").on(
      table.membershipId,
      table.onDate,
    ),
...
export const clients = pgTable(
  ...
  (table) => [
    index("client_org_name_idx").on(table.organizationId, table.name),
  ],
);
...
export const brands = pgTable(
  ...
  (table) => [
    uniqueIndex("brand_client_name_unique").on(table.clientId, table.name),
  ],
);
...
export const contactChannels = pgTable(
  ...
  (table) => [
    index("contact_channel_value_idx").on(table.organizationId, table.value),
  ],
);
...
export const rateItems = pgTable(
  "rate_items",
  {
    ...
    standardPriceMinor: bigint("standard_price_minor", {
      mode: "number",
    }).notNull(),
    effectiveFrom: date("effective_from").notNull(),
  ...
  (table) => [
    uniqueIndex("rate_item_effective_unique").on(
      table.rateCardId,
      table.serviceCode,
      table.effectiveFrom,
    ),
...
export const intakeItems = pgTable(
  ...
  (table) => [
    index("intake_org_status_idx").on(
      table.organizationId,
      table.status,
      table.createdAt,
    ),
...
export const intakeSourceItems = pgTable(
  "intake_source_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
      .notNull()
      .references(() => organizations.id),
    intakeItemId: uuid("intake_item_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("intake_provider_message_unique").on(
      table.organizationId,
      table.provider,
      table.providerMessageId,
    ),
    uniqueIndex("intake_source_sequence_unique").on(
      table.intakeItemId,
      table.sequence,
    ),
...
export const intakeAttachments = pgTable("intake_attachments", {
  ...
  sourceItemId: uuid("source_item_id")
    .notNull()
    .references(() => intakeSourceItems.id),
...
export const intakeSuggestions = pgTable(
  "intake_suggestions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
    ...
    confirmedByMembershipId: uuid("confirmed_by_membership_id").references(
      () => memberships.id,
    ),
    ...auditColumns,
  },
  (table) => [
    check(
      "suggestion_confidence_range",
      sql`${table.confidence} IS NULL OR (${table.confidence} >= 0 AND ${table.confidence} <= 10000)`,
    ),
  ],
);
...
export const projects = pgTable(
  "projects",
  {
    ...
    sourceIntakeItemId: uuid("source_intake_item_id").references(
      () => intakeItems.id,
    ),
  ...
  (table) => [
    index("project_org_status_deadline_idx").on(
      table.organizationId,
      table.status,
      table.deadline,
    ),
...
/** Reusable operational defaults. The definition intentionally contains no
 * people, files, comments, approvals, or audit identities. */
export const projectPacks = pgTable(
  ...
  (table) => [
    uniqueIndex("project_pack_org_name_unique").on(
      table.organizationId,
      table.name,
    ),
  ],
...
/** A saved change set is inert until a manager explicitly applies it. */
export const planningScenarios = pgTable(
  "planning_scenarios",
  {
    ...
    appliedByMembershipId: uuid("applied_by_membership_id").references(
      () => memberships.id,
    ),
    ...auditColumns,
  },
  (table) => [
    index("planning_scenario_project_idx").on(table.projectId, table.createdAt),
  ],
);
...
export const workflowStages = pgTable(
  ...
  (table) => [
    uniqueIndex("workflow_stage_position_unique").on(
      table.workflowId,
      table.position,
    ),
    uniqueIndex("one_client_review_stage_per_workflow")
      .on(table.workflowId)
      .where(sql`${table.semantic} = 'CLIENT_REVIEW'`),
...
export const tasks = pgTable(
  "tasks",
  {
    ...
    currentWorkflowStageId: uuid("current_workflow_stage_id").references(
      () => workflowStages.id,
    ),
  ...
  (table) => [
    ...
    check(
      "task_state_shape",
      sql`(${table.stateKind} = 'WORKFLOW' AND ${table.currentWorkflowStageId} IS NOT NULL) OR (${table.stateKind} <> 'WORKFLOW' AND ${table.currentWorkflowStageId} IS NULL)`,
    ),
    check(
...
export const taskAssignees = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("one_active_primary_owner_per_task")
      .on(table.taskId)
      .where(sql`${table.kind} = 'PRIMARY' AND ${table.removedAt} IS NULL`),
...
export const timeEntries = pgTable(
  ...
  (table) => [
    index("time_task_member_date_idx").on(
      table.taskId,
      table.membershipId,
      table.workDate,
    ),
...
export const fileVersions = pgTable(
  "file_versions",
  {
    ...
    processingStatus: varchar("processing_status", { length: 40 })
      .notNull()
      .default("QUEUED"),
    detectedContentType: varchar("detected_content_type", { length: 180 }),
    malwareStatus: varchar("malware_status", { length: 40 })
      .notNull()
      .default("PENDING"),
    processingFailureDetail: text("processing_failure_detail"),
  ...
  (table) => [
    uniqueIndex("file_version_number_unique").on(
      table.fileAssetId,
      table.versionNumber,
    ),
    uniqueIndex("file_version_storage_key_unique").on(
...
export const uploadSessions = pgTable(
  "upload_sessions",
  {
    ...
    uploadMode: varchar("upload_mode", { length: 20 })
      .notNull()
      .default("SINGLE"),
    expectedPartCount: integer("expected_part_count").notNull().default(1),
    uploaderMembershipId: uuid("uploader_membership_id")
      .notNull()
      .references(() => memberships.id),
  ...
  (table) => [
    index("upload_session_expiry_idx").on(table.status, table.expiresAt),
  ],
);

export const fileApprovals = pgTable(
  "file_approvals",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
    ...
    approvedAt: timestamp("approved_at", { withTimezone: true })
      ...
      .defaultNow(),
    reopenedAt: timestamp("reopened_at", { withTimezone: true }),
    reopenedByMembershipId: uuid("reopened_by_membership_id").references(
      () => memberships.id,
    ),
    reopenReason: text("reopen_reason"),
  },
...
export const reviewShares = pgTable(
  "review_shares",
  {
    ...
    organizationId: uuid("organization_id")
      .notNull()
      .references(() => organizations.id),
    reviewHubId: uuid("review_hub_id")
      .notNull()
      .references(() => reviewHubs.id),
    fileVersionId: uuid("file_version_id")
      .notNull()
      .references(() => fileVersions.id),
    tokenHash: varchar("token_hash", { length: 64 }).notNull().unique(),
    ...
    deliveryChannel: notificationChannelEnum("delivery_channel"),
    createdByMembershipId: uuid("created_by_membership_id")
      .notNull()
      .references(() => memberships.id),
  ...
  (table) => [
    index("review_share_hub_status_idx").on(table.reviewHubId, table.status),
  ],
);
...
export const reviewerSessions = pgTable("reviewer_sessions", {
  ...
  organizationId: uuid("organization_id")
    .notNull()
    .references(() => organizations.id),
  reviewShareId: uuid("review_share_id")
    .notNull()
    .references(() => reviewShares.id),
...
export const reviewComments = pgTable(
  "review_comments",
  {
    ...
    fileVersionId: uuid("file_version_id")
      .notNull()
      .references(() => fileVersions.id),
    reviewerSessionId: uuid("reviewer_session_id").references(
      () => reviewerSessions.id,
    ),
    internalAuthorMembershipId: uuid(
      "internal_author_membership_id",
    ).references(() => memberships.id),
    parentCommentId: uuid("parent_comment_id"),
    body: text("body").notNull(),
  ...
  (table) => [
    index("review_comment_share_created_idx").on(
      table.reviewShareId,
      table.createdAt,
    ),
    check(
      "review_comment_author_xor",
      sql`num_nonnulls(${table.reviewerSessionId}, ${table.internalAuthorMembershipId}) = 1`,
    ),
  ],
);
...
export const reviewViewEvents = pgTable(
  "review_view_events",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
  ...
  (table) => [
    index("review_view_share_time_idx").on(table.reviewShareId, table.viewedAt),
  ],
);

export const reviewCommentRevisions = pgTable(
  ...
  (table) => [
    index("review_comment_revision_idx").on(
      table.reviewCommentId,
      table.createdAt,
    ),
  ],
);
...
export const assetRights = pgTable(
  "asset_rights",
  {
    ...
    territory: varchar("territory", { length: 240 }).notNull(),
    ...
    documentFileVersionId: uuid("document_file_version_id").references(
      () => fileVersions.id,
    ),
    notes: text("notes"),
    createdByMembershipId: uuid("created_by_membership_id")
  ...
  (table) => [
    index("asset_right_expiry_idx").on(table.organizationId, table.validUntil),
    check(
      "asset_right_date_order",
...
export const quotes = pgTable(
  ...
  (table) => [
    check(
      "quote_parent_xor",
      sql`num_nonnulls(${table.projectId}, ${table.proposalId}) = 1`,
    ),
  ],
);
...
export const quoteVersions = pgTable(
  "quote_versions",
  {
    ...
    finalizedByMembershipId: uuid("finalized_by_membership_id").references(
      () => memberships.id,
    ),
    pdfStorageKey: text("pdf_storage_key"),
    pdfChecksumSha256: varchar("pdf_checksum_sha256", { length: 64 }),
    legalSnapshot: jsonb("legal_snapshot").notNull().default({}),
  ...
  (table) => [
    uniqueIndex("quote_version_number_unique").on(
      table.quoteId,
      table.versionNumber,
    ),
    check(
      "quote_version_totals_nonnegative",
      sql`${table.subtotalMinor} >= 0 AND ${table.discountMinor} >= 0 AND ${table.taxMinor} >= 0 AND ${table.totalMinor} >= 0`,
    ),
  ],
);
...
export const quoteLines = pgTable(
  ...
  (table) => [
    uniqueIndex("quote_line_position_unique").on(
    ...
    check(
      ...
      sql`${table.unitRateMinor} >= 0 AND ${table.discountBasisPoints} BETWEEN 0 AND 10000 AND ${table.taxBasisPoints} BETWEEN 0 AND 10000 AND ${table.lineTotalMinor} >= 0`,
    ),
  ],
);

export const quoteAcceptanceLinks = pgTable(
  "quote_acceptance_links",
  {
    ...
    revokedAt: timestamp("revoked_at", { withTimezone: true }),
    acceptedAt: timestamp("accepted_at", { withTimezone: true }),
    acceptedName: varchar("accepted_name", { length: 160 }),
    acceptedEmail: varchar("accepted_email", { length: 320 }),
  ...
  (table) => [
    index("quote_acceptance_version_status_idx").on(
      table.quoteVersionId,
      table.status,
    ),
    check(
      "quote_acceptance_state_shape",
      sql`(${table.status} = 'ACCEPTED' AND ${table.acceptedAt} IS NOT NULL AND ${table.acceptedName} IS NOT NULL AND ${table.acceptedEmail} IS NOT NULL AND ${table.evidenceSnapshot} IS NOT NULL) OR (${table.status} <> 'ACCEPTED' AND ${table.acceptedAt} IS NULL)`,
    ),
  ],
);

export const quoteAcceptanceEvents = pgTable(
  "quote_acceptance_events",
  {
    ...
    organizationId: uuid("organization_id")
      .notNull()
      .references(() => organizations.id),
    acceptanceLinkId: uuid("acceptance_link_id")
      .notNull()
      .references(() => quoteAcceptanceLinks.id),
  ...
  (table) => [
    index("quote_acceptance_event_idx").on(
      table.acceptanceLinkId,
      table.createdAt,
    ),
  ],
);
...
/**
 * Shared fixed-window counters for unauthenticated boundaries.  Keeping these
 * in Postgres (rather than process memory) makes the limit apply across Cloud
 * Run instances and survives a process restart.
 */
export const publicRateLimitBuckets = pgTable(
  ...
  (table) => [
    ...
    index("public_rate_limit_expiry_idx").on(table.windowStartedAt),
  ],
);

/** One-way ERP → Google Calendar projection. ERP dates always remain authoritative. */
export const calendarSyncRecords = pgTable(
  ...
  (table) => [
    uniqueIndex("calendar_sync_object_unique").on(table.integrationConnectionId, table.objectType, table.objectId),
    index("calendar_sync_status_idx").on(table.organizationId, table.status, table.updatedAt),
  ],
);
...
export const outboxEvents = pgTable(
  ...
  (table) => [
    index("outbox_dispatch_idx").on(table.dispatchedAt, table.availableAt),
  ],
);
...
export const jobRuns = pgTable(
  ...
  (table) => [
    index("job_queue_status_available_idx").on(
      table.queue,
      table.status,
      table.availableAt,
    ),
...
export const jobAttempts = pgTable(
  ...
  (table) => [
    uniqueIndex("job_attempt_number_unique").on(
      table.jobRunId,
      table.attemptNumber,
    ),
  ],
...
export const archiveJobs = pgTable(
  ...
  (table) => [
    index("archive_job_project_idx").on(table.projectId, table.createdAt),
  ],
);

export const archiveManifestEntries = pgTable(
  "archive_manifest_entries",
  {
    ...
    archiveJobId: uuid("archive_job_id")
      .notNull()
      .references(() => archiveJobs.id),
    fileVersionId: uuid("file_version_id")
      .notNull()
      .references(() => fileVersions.id),
    sourceStorageKey: text("source_storage_key").notNull(),
    destinationStorageKey: text("destination_storage_key").notNull(),
    expectedChecksumSha256: varchar("expected_checksum_sha256", {
      length: 64,
    }).notNull(),
    verifiedChecksumSha256: varchar("verified_checksum_sha256", { length: 64 }),
    status: varchar("status", { length: 40 }).notNull().default("PENDING"),
    failureDetail: text("failure_detail"),
    ...auditColumns,
  },
  (table) => [
    uniqueIndex("archive_manifest_job_version_unique").on(
      table.archiveJobId,
      table.fileVersionId,
    ),
  ],
...
export const projectClosureChecklistItems = pgTable(
  "project_closure_checklist_items",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("project_closure_checklist_key_unique").on(
      table.projectId,
      table.key,
    ),
  ],
...
export const projectClosureEvents = pgTable(
  ...
  (table) => [
    index("project_closure_event_idx").on(table.projectId, table.createdAt),
  ],
);

export const projectRetrospectives = pgTable(
  "project_retrospectives",
  {
    ...
    deadlineSummary: jsonb("deadline_summary").notNull().default({}),
    revisionSummary: jsonb("revision_summary").notNull().default({}),
    bottleneckSummary: text("bottleneck_summary").notNull().default(""),
  ...
  (table) => [uniqueIndex("project_retrospective_unique").on(table.projectId)],
);

export const templateImprovementSuggestions = pgTable(
  "template_improvement_suggestions",
  {
    ...
    retrospectiveId: uuid("retrospective_id")
      .notNull()
      .references(() => projectRetrospectives.id),
    projectPackId: uuid("project_pack_id").references(() => projectPacks.id),
  ...
  (table) => [
    index("template_suggestion_retro_idx").on(
...
export const intakeConversions = pgTable(
  "intake_conversions",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    organizationId: uuid("organization_id")
    ...
    idempotencyKey: varchar("idempotency_key", { length: 200 })
      .notNull()
      .unique(),
    convertedByMembershipId: uuid("converted_by_membership_id")
      .notNull()
      .references(() => memberships.id),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    ...
    check(
      "intake_conversion_one_target",
      sql`num_nonnulls(${table.proposalId}, ${table.projectId}, ${table.taskId}) = 1`,
    ),
  ],
);

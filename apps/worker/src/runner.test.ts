...
it("completes idempotent jobs after the handler succeeds", async () => {
  ...
  const queue: JobQueue = {
    enqueue: async () => {}, claim: async <T>() => claims++ === 0 ? [job as JobEnvelope<T>] : [], recordAttempt: async (value) => { attempts.push(value.id); }, complete: async (value) => { completed.push(value.id); }, retry: async () => {}, fail: async () => {}, health: async () => ({ queues: [] }),
  };
  ...
  await worker.run();
  expect(attempts).toEqual([job.id]);

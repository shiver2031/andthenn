...
export async function POST(request: Request) {
  ...
  if (!process.env.DATABASE_URL) return NextResponse.json({ accepted: true, demo: true }, { status: 202 });
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(outboxEvents).values({ organizationId: process.env.ORGANIZATION_ID!, eventType: "gmail.reconcile", aggregateType: "MAILBOX", aggregateId: process.env.ORGANIZATION_ID!, payload: { historyCursor: notification.historyId }, idempotencyKey: `gmail:${parsed.data.message.messageId}`, correlationId }).onConflictDoNothing();
  });

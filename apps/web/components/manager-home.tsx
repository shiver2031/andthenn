...
export function ManagerHome() {
  ...
  return <>
    <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-violet-600">Tuesday · 04 August</p><h1 className="display text-3xl font-bold md:text-4xl">Good morning, Maya.</h1><p className="mt-2 text-sm text-zinc-600">Here’s what needs your attention across the studio.</p></div><Button asChild><Link href="/intake"><Inbox size={16} /> Open intake <span className="rounded-full bg-white/20 px-1.5 text-[10px]">4</span></Link></Button></div>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[1.55fr_1fr]">
      <section className="surface overflow-hidden rounded-2xl"><div className="flex items-center justify-between border-b border-zinc-100 p-5"><div><h2 className="display text-lg font-bold">Project pulse</h2><p className="mt-1 text-xs text-zinc-600">Live delivery health, by deadline</p></div><Link href="/projects" className="text-xs font-bold text-violet-700 hover:text-violet-900">View all</Link></div>
        <div className="divide-y divide-zinc-100">{projects.map((project, index) => <motion.div key={project.id} initial={reduced ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * .06 }} className="grid gap-4 p-5 hover:bg-zinc-50/60 sm:grid-cols-[1fr_130px_100px] sm:items-center">
          ...
          <div className="flex items-center justify-between sm:block sm:text-right"><Badge tone={project.tone === "amber" ? "amber" : "green"}>{project.health}</Badge><p className="mt-1 text-[10px] text-zinc-600">Due {project.due}</p></div>
        </motion.div>)}</div>
      </section>

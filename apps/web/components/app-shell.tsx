...
function Brand() {
  return <Link href="/home" className="group flex items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400"><span className="contents">
    ...
    <span><span className="display block text-[17px] font-bold tracking-tight text-white">AndThenn</span><span className="block text-[10px] font-semibold uppercase tracking-[.18em] text-zinc-500">Media ERP</span></span>
  </span></Link>;
}
  ...
  return <aside className="flex h-full w-[248px] flex-col bg-[#11121a] px-3 py-4 text-zinc-300">
    ...
    <nav aria-label="Primary" className="flex-1 space-y-6">
      {sections.map((section) => <div key={section.label}>
        <p className="px-3 pb-2 text-[10px] font-bold uppercase tracking-[.18em] text-zinc-600">{section.label}</p>
        <div className="space-y-1">{section.links.map(({ href, label, icon: Icon, badge }) => {
          ...
          return <Link {...(close ? { onClick: close } : {})} key={href} href={href} className={cn("relative flex min-h-11 items-center gap-3 rounded-xl px-3 text-[13px] font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400", active ? "bg-white/10 text-white" : "text-zinc-500 hover:bg-white/[.06] hover:text-zinc-200")}>
    ...
    <div className="space-y-1 border-t border-white/[.07] pt-3">
      <Link href="/admin" className="flex min-h-10 items-center gap-3 rounded-xl px-3 text-xs font-semibold text-zinc-500 hover:bg-white/[.06] hover:text-white"><Settings size={16} /> Settings</Link>
      <button className="flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-xs font-semibold text-zinc-500 hover:bg-white/[.06] hover:text-white"><CircleHelp size={16} /> Help & support</button>
      <button className="mt-2 flex w-full items-center gap-3 rounded-xl bg-white/[.045] p-2 text-left hover:bg-white/[.08]">
        ...
        <span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold text-zinc-200">Maya Shah</span><span className="block text-[10px] text-zinc-600">Manager</span></span><ChevronDown size={14} />
      </button>
    </div>
  </aside>;
}
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
    ...
    <div className="min-w-0">
      <header className="sticky top-0 z-30 flex h-16 items-center border-b border-zinc-200/80 bg-[#f6f5f1]/90 px-4 backdrop-blur-xl md:px-7">
        ...
        <button onClick={() => setSearchOpen(true)} className="flex h-10 min-w-0 max-w-lg flex-1 items-center gap-2 rounded-xl border border-zinc-200 bg-white px-3 text-left text-sm text-zinc-400 shadow-sm hover:border-zinc-300"><Search className="shrink-0" size={17} /><span className="min-w-0 flex-1 truncate">Search tasks, projects, clients…</span><kbd className="hidden rounded-md border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] text-zinc-500 sm:block">⌘ K</kbd></button>
        <div className="ml-2 flex shrink-0 items-center gap-1.5 sm:ml-auto sm:pl-4">
          <Button size="sm" className="hidden sm:inline-flex"><Plus size={15} /> New</Button>
    ...
    <AnimatePresence>{searchOpen && <motion.div role="dialog" aria-modal="true" aria-label="Global search" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] grid place-items-start bg-zinc-950/35 px-4 pt-[12vh] backdrop-blur-sm" onClick={() => setSearchOpen(false)}>
      <motion.div initial={{ y: -12, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: -12, scale: .98 }} className="w-full max-w-2xl overflow-hidden rounded-2xl border border-white/60 bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center gap-3 border-b border-zinc-100 px-5"><Search size={20} className="text-violet-500" /><input autoFocus aria-label="Search" placeholder="Search all of AndThenn…" className="h-16 flex-1 outline-none" /><button aria-label="Close search" onClick={() => setSearchOpen(false)}><X size={18} /></button></div>
        <div className="p-3"><p className="px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-zinc-400">Jump to</p>{[[FolderKanban,"Aster House — Monsoon Stories","Project · AT-042"],[BriefcaseBusiness,"Hero film — edit V2","Task · Due today"],[Building2,"Aster House","Client"]].map(([Icon,title,meta]) => { const C = Icon as LucideIcon; return <button key={String(title)} className="flex w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-zinc-50"><span className="grid size-9 place-items-center rounded-lg bg-violet-50 text-violet-600"><C size={17} /></span><span><span className="block text-sm font-semibold">{String(title)}</span><span className="text-xs text-zinc-400">{String(meta)}</span></span></button>})}</div>
      </motion.div>

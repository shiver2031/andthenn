...
export function LoginForm({ prototype = false }: { prototype?: boolean }) {
  ...
  if (prototype) return <div className="mt-8 space-y-3" aria-describedby="prototype-note">
    ...
    {([[
      ...
      <button key={persona} disabled={busy} onClick={() => prototypeLogin(persona)} className="group flex min-h-11 w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[.04] px-3 text-left transition hover:border-violet-300/60 hover:bg-white/[.08] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-300 disabled:cursor-not-allowed disabled:opacity-50"><span className="grid size-9 shrink-0 place-items-center rounded-lg bg-violet-400/15 text-violet-200"><UserRound size={17}/></span><span><span className="block text-sm font-semibold text-white">{title}</span><span className="block text-xs text-zinc-400">{detail}</span></span></button>
    ))}
    {message && <p role="status" className="text-center text-xs text-zinc-300">{message}</p>}
  </div>;
